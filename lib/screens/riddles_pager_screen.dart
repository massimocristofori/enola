...
class _RiddlesPagerScreenState extends State<RiddlesPagerScreen> {
  ...
  Future<void> _loadRiddles() async {
    ...
    // if no riddles yet, insert a blank one immediately
    if (_riddles.isEmpty) {
  ...
  Future<void> _insertBlankRiddle(
    ...
    final db = DriftService.instance.db;
    // shift orderInMap of existing riddles after insertion point
    ...
    final companion = RiddlesCompanion.insert(
      ...
      orderInMap: atIndex,
      payloadJson: const drift.Value(null),
      choicesJson: const drift.Value(null),
      correctIndex: const drift.Value(null),
    );
    final id = await db.into(db.riddles).insert(companion);
  ...
  Future<void> _deleteRiddle(int riddleIndex) async {
    ...
    final db = DriftService.instance.db;
    await (db.delete(db.riddles)..where((t) => t.id.equals(riddle.id))).go();
    setState(() => _riddles.removeAt(riddleIndex));

    // fix orderInMap for remaining
    await _persistOrder();

    final newPage = (_currentPage >= _riddles.length && _riddles.isNotEmpty)
        ? _riddles.length - 1
        : _currentPage;
  ...
  Future<void> _persistOrder() async {
    ...
    await db.batch((batch) {
      for (int i = 0; i < _riddles.length; i++) {
        batch.update(
          db.riddles,
          RiddlesCompanion(orderInMap: drift.Value(i)),
          where: (t) => t.id.equals(_riddles[i].id),
        );
      }
    });
  ...
  Widget build(BuildContext context) {
    if (_loading) {
      return const Scaffold(
        body:
            Center(child: CircularProgressIndicator(color: EnolaTheme.accent)),
      );
    }
              ...
              child: _riddles.isEmpty
                  ...
                  : PageView.builder(
                      ...
                      itemBuilder: (context, index) => _RiddleEditorPage(
                        ...
                        mapId: widget.mapId,
                        onSaved: (updated) => _onRiddleSaved(index, updated),
...
class _RiddleEditorPageState extends State<_RiddleEditorPage> {
    ...
    try {
      ...
      switch (_selectedType) {
        ...
        case RiddleType.trueFalse:
          ...
          correctIndex = _tfCorrectIndex;

        case RiddleType.ordering:
          final payload = OrderingPayload(items: _orderItems);
          payloadJson = jsonEncode(payload.toJson());
          choicesJson = jsonEncode(_orderItems);
          correctIndex = null;
      }
      ...
      await (db.update(db.riddles)
        ...
        question: drift.Value(_qCtrl.text.trim()),
        typeIndex: drift.Value(_selectedType.index),
        payloadJson: drift.Value(payloadJson),
        choicesJson: drift.Value(choicesJson),
        correctIndex: drift.Value(correctIndex),
      ));

      final updated = await (DriftService.instance.db.select(
  ...
  Widget _tfOption(int index, String label, IconData icon) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: selected ? EnolaTheme.correct.withAlpha(20) : EnolaTheme.surface,
          borderRadius: BorderRadius.circular(14),
          border: Border.all(
  ...
  Widget _buildOrderingForm() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('ITEMS IN CORRECT ORDER',
            style: EnolaTheme.sectionHeader),
        const SizedBox(height: 8),
        Row(
          children: [
  ...
  Widget _addItemField(String hint) {
    return TextField(
      ...
      style: const TextStyle(fontSize: 14, color: EnolaTheme.textPrimary),
      decoration: InputDecoration(
        hintText: hint,
        hintStyle:
            TextStyle(color: EnolaTheme.textSecond.withAlpha(120), fontSize: 14),
        filled: true,
        fillColor: EnolaTheme.surface,
        contentPadding:
        ...
        focusedBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(12),
          borderSide: const BorderSide(color: EnolaTheme.accent, width: 1.5),
        ),
      ),
    );
  }
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Text('Delete Riddle?',
            style: TextStyle(
                color: EnolaTheme.textPrimary, fontWeight: FontWeight.w700)),
        ...
        actions: [
          ...
          TextButton(
            ...
            child: const Text('Delete',
                style: TextStyle(
                    color: EnolaTheme.wrong, fontWeight: FontWeight.w700)),
          ),
        ],
      ),
    );
